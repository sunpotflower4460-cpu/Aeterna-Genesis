...
This module asks a deliberately weaker question than "is this a universal law?": do independently
implemented Worlds show similarly shaped *observable transitions* when expressed in a small common
vocabulary?  It never promotes a Room, assigns an official Emergence Level, changes a hypothesis
confidence, or treats a shared fingerprint as proof of identical physics.

The comparator has two layers:
1) rerun each already-selected shadow World/Zero pair with a small common-observable time series;
2) project recurrent g001 X-patterns into that common vocabulary and look for fingerprint overlap.

Start purity is kept explicit.  A scaffolded g001 source matching a Z-A run in another World is only
SIGNATURE_OVERLAP_ONLY, never a strict zero-aligned cross-world candidate.
"""
...
def _magnitude_and_alignment_field(field: np.ndarray, kind: str) -> tuple[np.ndarray, np.ndarray]:
    arr = np.asarray(field)
    if kind == "complex":
        mag = np.abs(arr)
        unit = np.where(mag > 1e-12, arr / np.maximum(mag, 1e-12), 0.0j)
        return mag, unit
    if kind == "scalar":
        mag = np.abs(arr)
        unit = np.where(mag > 1e-12, arr / np.maximum(mag, 1e-12), 0.0)
        return mag, unit
    if kind in {"vector", "nematic"}:
        mag = np.linalg.norm(arr, axis=-1)
        unit = np.where(mag[..., None] > 1e-12, arr / np.maximum(mag[..., None], 1e-12), 0.0)
        return mag, unit
    raise ValueError(f"unsupported common observable kind: {kind}")


def _spectral_features(mag: np.ndarray) -> tuple[float, float, float]:
    field = np.asarray(mag, dtype=float)
...
def _snapshot(field: np.ndarray, *, kind: str, physical_time: float) -> dict[str, float]:
    mag, unit = _magnitude_and_alignment_field(field, kind)
    mean = float(np.mean(mag))
    std = float(np.std(mag))
    if kind == "complex":
        alignment = float(abs(np.mean(unit)))
    elif kind == "scalar":
        alignment = float(abs(np.mean(unit)))
    else:
        axes = tuple(range(unit.ndim - 1))
        alignment = float(np.linalg.norm(np.mean(unit, axis=axes)))
    entropy, k_rms, anis = _spectral_features(mag)
    gy = np.roll(mag, -1, axis=0) - mag
    gx = np.roll(mag, -1, axis=1) - mag
    grad = float(np.sqrt(np.mean(gx * gx + gy * gy)))
    high = float(np.mean(mag > mean + std))
    return {
...
def _common_probe_g001(zero_id: str, seed: int, quick: bool) -> dict[str, Any]:
    ...
    snaps = [_snapshot(psi, kind="complex", physical_time=0.0)]
    every = max(1, steps // 18)
    finite = True
    for i in range(1, steps + 1):
        psi = gl.step(psi, (i - 1) * p["dt"], p)
        if not np.isfinite(psi).all():
            finite = False
            break
        if i % every == 0 or i == steps:
            snaps.append(_snapshot(psi, kind="complex", physical_time=i * p["dt"]))
    return {
        "world_id": "g001-tdgl", "zero_id": zero_id, "seed": seed, "finite": finite,
        "analysis_start_time": max(float(p.get("quench_duration", 0.0)), steps * p["dt"] * 0.2),
        "snapshots": snaps,
    }


def _common_probe_model_h(zero_id: str, seed: int, quick: bool) -> dict[str, Any]:
    if zero_id != "Z-A":
    ...
    s = model_h.ModelH(N, seed=seed, coupling=1.0)
    snaps = [_snapshot(s.phi(), kind="scalar", physical_time=0.0)]
    every = max(1, steps // 18)
    finite = True
    for i in range(1, steps + 1):
        s.step(dt)
        if not s.finite():
            ...
            break
        if i % every == 0 or i == steps:
            snaps.append(_snapshot(s.phi(), kind="scalar", physical_time=i * dt))
    return {
        "world_id": "g003-model-h", "zero_id": zero_id, "seed": seed, "finite": finite,
        # Model H has no externally imposed time-dependent quench. Skip only the first small numerical transient.
        "analysis_start_time": steps * dt * 0.1,
        "snapshots": snaps,
    }


def _common_probe_o3(zero_id: str, seed: int, quick: bool) -> dict[str, Any]:
    if zero_id not in {"Z-A", "Z-B"}:
    ...
    snaps = [_snapshot(phi, kind="vector", physical_time=0.0)]
    every = max(1, steps // 18)
    finite = True
    for i in range(1, steps + 1):
        phi = o3.step(phi, (i - 1) * p["dt"], p)
        if not np.isfinite(phi).all():
            finite = False
            break
        if i % every == 0 or i == steps:
            snaps.append(_snapshot(phi, kind="vector", physical_time=i * p["dt"]))
    return {
        "world_id": "o3-vector", "zero_id": zero_id, "seed": seed, "finite": finite,
        "analysis_start_time": max(float(p.get("quench_duration", 0.0)), steps * p["dt"] * 0.2),
        "snapshots": snaps,
    }


def _common_probe_q2(zero_id: str, seed: int, quick: bool) -> dict[str, Any]:
    if zero_id not in {"Z-A", "Z-B"}:
    ...
    snaps = [_snapshot(q, kind="nematic", physical_time=0.0)]
    every = max(1, steps // 18)
    finite = True
    for i in range(1, steps + 1):
        q = q2.step(q, (i - 1) * p["dt"], p)
        if not np.isfinite(q).all():
            finite = False
            break
        if i % every == 0 or i == steps:
            snaps.append(_snapshot(q, kind="nematic", physical_time=i * p["dt"]))
    return {
        "world_id": "q2-nematic", "zero_id": zero_id, "seed": seed, "finite": finite,
        "analysis_start_time": max(float(p.get("quench_duration", 0.0)), steps * p["dt"] * 0.2),
        "snapshots": snaps,
    }
...
def common_probe(world_id: str, *, zero_id: str, seed: int, quick: bool = True) -> dict[str, Any]:
    fn = {
        "g001-tdgl": _common_probe_g001,
        "g003-model-h": _common_probe_model_h,
        "o3-vector": _common_probe_o3,
        "q2-nematic": _common_probe_q2,
    }.get(world_id)
    if fn is None:
        raise ValueError(f"world {world_id} has no common-observable probe")
    out = fn(zero_id, int(seed), bool(quick))
    zero = get_zero(zero_id)
    out["zero_purity_class"] = zero.purity_class
    out["strict_zero_candidate"] = bool(zero.strict_zero_candidate)
    out["imposed_length_scale"] = bool(zero.imposed_length_scale)
    return out
...
def _fingerprint(delta: np.ndarray) -> str | None:
    ...
    for i in ranked:
        ...
        if abs(v) < 0.15:
            continue
        parts.append(f"{COMMON_FEATURES[i]}:{'+' if v > 0 else '-'}{_bucket(abs(v))}")
        if len(parts) >= 5:
            break
...
def detect_common_episodes(probe: dict[str, Any], *, max_episodes: int = 3) -> list[dict[str, Any]]:
    ...
    delta = np.diff(matrix, axis=0) / scales
    ...
    for i in sorted(chosen):
        ...
        if not fp:
            continue
        pid = "CWX-" + hashlib.sha256(fp.encode()).hexdigest()[:10]
        episodes.append({
            "pattern_id": pid,
            "fingerprint": fp,
...
def project_g001_fingerprint(fingerprint: str | None) -> dict[str, Any]:
    parts = [p for p in str(fingerprint or "").split("|") if p]
    mapped: list[str] = []
    dropped: list[str] = []
    for part in parts:
        if ":" not in part:
    ...
    projected = "|".join(mapped) if mapped else None
    return {
        "fingerprint": projected,
        "coverage": 0.0 if not parts else len(mapped) / len(parts),
        "mapped_parts": mapped,
...
def _g001_purities(ledger: dict[str, Any], pattern_id: str, pattern: dict[str, Any]) -> list[str]:
    ...
    if rep.get("zero_purity"):
        purities.append(str(rep["zero_purity"]))
    return list(dict.fromkeys(purities))


def _strict_alignment(source_purities: list[str], target_episodes: list[dict[str, Any]]) -> bool:
    source_za = any(x.startswith("Z-A:") for x in source_purities)
    target_za = any(e.get("zero_id") == "Z-A" and e.get("strict_zero_candidate") for e in target_episodes)
...
def _update_cross_ledger(*, burst_id: str, episodes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    ordered = sorted(patterns.values(), key=lambda p: (len(p.get("worlds") or []), len(p.get("seeds") or []), p.get("observations", 0)), reverse=True)[:256]
    ledger.update({
        "patterns": ordered,
        "last_burst": burst_id,
        "note": "Shared common-observable fingerprints are navigation leads, not universality or identical-physics claims.",
    })
...
def compare_g001_patterns(*, episodes: list[dict[str, Any]], g001_ledger: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for pattern in g001_ledger.get("patterns") or []:
        if pattern.get("status") not in {
            "REPEATED", "CROSS_CONDITION_RECURRENT", "ROBUST_RECURRENT_CANDIDATE", "CROSS_WORLD_CANDIDATE"
        }:
            continue
        proj = project_g001_fingerprint(pattern.get("fingerprint"))
        fp = proj.get("fingerprint")
        if not fp or fp not in by_fp:
            continue
        target = by_fp[fp]
        purities = _g001_purities(g001_ledger, str(pattern["pattern_id"]), pattern)
        aligned = _strict_alignment(purities, target)
        coverage = float(proj.get("coverage", 0.0))
        if aligned and coverage >= 0.999:
            status = "CROSS_WORLD_ZERO_ALIGNED_LEAD"
        else:
            status = "SIGNATURE_OVERLAP_ONLY"
        matches.append({
            ...
            "dropped_g001_features": proj.get("dropped_parts") or [],
            "g001_start_purities": purities,
            "matched_worlds": sorted({str(e.get("world_id")) for e in target}),
            "matched_world_zero_pairs": sorted({f"{e.get('world_id')}@{e.get('zero_id')}" for e in target}),
            "matched_observations_this_burst": len(target),
            "strict_ZA_alignment": aligned,
            "status": status,
            "universality_claim": False,
            "identical_physics_claim": False,
        })
    matches.sort(key=lambda x: (x["strict_ZA_alignment"], len(x["matched_worlds"]), x["matched_observations_this_burst"]), reverse=True)
    return matches


def analyze_shadow_report(report: dict[str, Any], *, quick: bool = True, max_episodes_per_probe: int = 3) -> dict[str, Any]:
    common_probes: list[dict[str, Any]] = []
    errors: list[dict[str, str]] = []
    for obs in report.get("observations") or []:
        try:
            common_probes.append(common_probe(
    ...
    for p in common_probes:
        if p.get("finite"):
            episodes.extend(detect_common_episodes(p, max_episodes=max_episodes_per_probe))
    ...
    summary = {
        ...
        "probes": len(common_probes),
        "finite_probes": sum(bool(p.get("finite")) for p in common_probes),
        ...
        "note": (
            "Common fingerprints compare the shape of observable changes after per-run normalization. "
            "They do not equate order parameters, defect physics, conserved quantities, or microscopic laws."
        ),
    }
