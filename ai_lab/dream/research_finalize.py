...
3. Research Backlog preserves burst-local instrument/infrastructure debt for future agents.
4. Research Manifest hashes the final evidence/planning/view state into an immutable burst record.
...
def run(*, persist: bool = True) -> dict:
    ...
    backlog = research_backlog.run(persist=persist)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Finalize Compass -> Health -> Backlog -> immutable provenance manifest")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"ops_backlog={backlog.get('active_count')} manifest={manifest.get('manifest_content_sha256')}"
    )
