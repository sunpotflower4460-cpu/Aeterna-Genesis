#!/usr/bin/env python3
"""ai_lab/relational/run.py -- CLI for the R-layer substrate + instruments R1-R4, R7
(PR-R1 + PR-R1.5 + PR-R1.75 + PR-R1.9 + PR-R2.1 + PR-R2.2).

MODULE:      relational_r1 (ai_lab/relational, PR-R1 through PR-R2.2 addenda)
QUESTION:    Starting from only node indices, a relation graph, and a real-valued per-node
             ...
             ai_lab/relational/verify.py rather than caught after the fact each time). This
             is DAMAGE-RECOVERY ACHIEVED (AUDIT.md Sec.14.1, S-011): a self-sustaining
             attracting limit cycle, confirmed by perturbing its amplitude and watching it
             return, not merely observed to persist. PR-R2.2 corrects the target N to the
             `damping=0.05`-only subset (100 runs / 223 node-checks, not 240/663, AUDIT.md
             Sec.14.2) and builds R7 (phase, instruments.py::phase()) on it, with a fixed
             rule trimming the initial transient itself (not just the Hilbert edge, AUDIT.md
             Sec.14.4). Cycle-clustering (AUDIT.md Sec.14.3) is reframed, not retracted:
             verified oscillation localizes near hub-like/degree-heterogeneous structure,
             and at least one fully-covered cycle exists in the `damping=0.05` verified
             data -- a small, concrete opening for a future R8 (not built this PR).
...
AUDIT (7):   1. Rule names the result?                 No -- the update rule, the asymmetry
             ...
             4. Untargeted companion phenomena appear?   Partial -- R1-R4 and R7 exist; R5,
                R6, R8-R11 do not. PR-R2.2 built R7 (phase) on the corrected N (damping=0.05,
                saturation="cubic", verify_long_window-confirmed: 100 runs / 223 node-checks,
                AUDIT.md Sec.14.2) with a fixed transient-trim rule (Sec.14.4), and found a
                concrete (if small) opening for a future R8: at least one fundamental cycle
                is fully covered by verified nodes in this corrected data (Sec.14.3) --
                R8 itself was not built. The cycle-clustering question is reopened as a
                positive finding (localizes near hub-like structure), not closed.
             5. Matches reality with real numbers?        Yes for all three proven cells
                ...
                positive saturation="cubic" result: "measured" the damping=0.05-specific
                100/223 verified count, "confirmed" (not just measured) the damage-recovery
                split via a disclosed sample, and (PR-R2.2) "measured" R7's phase-unwrapping
                rate independently agreeing with R4's autocorrelation rate to within 3-5% on
                the validated example -- three different lines of evidence agreeing, not one
                number reported three times.
             6. Robust to changing IC/parameters?         Yes -- the full PR-R1 through
                PR-R2.2 sweep history (AUDIT.md Sec.3-14) spans thousands of seed/topology/
                strength/damping/saturation combinations; PR-R2.1 re-swept the full
                600-config grid under saturation="cubic"; PR-R2.2 re-derived the
                damping=0.05-only breakdown and cycle-coverage figures from that same sweep
                without re-sampling.
             7. Code asserts or discovers the conclusion? Discovers -- run.py/verify.py/
                instruments.py report `sustained`, `settled`, `sustained_and_settled`,
                `verified_sustained`, `achieved` (damage-recovery), and now R7's per-node
                `defined`/`unwrapped_phase_span`/`mean_rate_from_phase` as separate, explicit
                fields; nothing upgrades a weaker verdict into a stronger-sounding one.
                PR-R2.1's retraction of Sec.10.4/11's saturation="none" headline, and its
                reversal of Sec.12.3's "not a hub artifact" claim (PR-R2.2, Sec.14.3:
                reframed as hub-localization, not silently dropped), were both written into
                AUDIT.md with the same weight as the positive findings -- corrections and
                reframings, not omissions.
STATUS:      YELLOW, upgraded in substance though not in color: the positive claim is now at
             the "measured, 15x-window-verified, AND interventionally ACHIEVED" tier for the
             first time in this PR series (damping=0.05, saturation="cubic" -- damage-
             recovery, AUDIT.md Sec.14.1, S-011), and R7 (phase) is now built on the
             corrected N with a cross-checked, agreeing measurement. Not GREEN: R5, R6, and
             R8-R11 still do not exist; R8's precondition is met only on 3 small, disclosed
             example cycles so far (Sec.14.3), not a swept characterization; the
             damage-recovery sample is n=16, not exhaustive, and its 2/8 damping=0.0
             exceptions are recorded as open, not explained (Sec.14.5); and the
             q^2>p*gamma^2 cross-validation is still limited to one topology/seed pair
             (Sec.10.5, unchanged since PR-R1.75).
