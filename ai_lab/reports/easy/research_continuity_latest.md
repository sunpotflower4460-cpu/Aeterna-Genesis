...
latest strict burst: `dream-20260827-0488`
continuity digest: `b79cac525ddb575544af`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=2/5, control=10/12, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=26, pair-only=22, triads=19, split-asym=0.037559, no-split-asym=0.171838, energy-before-geometry=2. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=10/26, nearby=?, control=0/26. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=61/77, nearby=?, control=0/77. 回数だけでなく、何を変えると消えるかを追う。
