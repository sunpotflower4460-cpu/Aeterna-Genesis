...
latest strict burst: `dream-20260916-0964`
continuity digest: `44495c7e8239feb40751`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=2/5, control=10/11, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=30, pair-only=25, triads=16, split-asym=0.059387, no-split-asym=0.056875, energy-before-geometry=1. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=31/65, nearby=?, control=0/65. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-deea6a8f25` [strict/open-ended-followup] — X-deea6a8f25: status=REPEATED_SPECIFIC_CANDIDATE; same=22/61, nearby=?, control=0/61. 回数だけでなく、何を変えると消えるかを追う。
