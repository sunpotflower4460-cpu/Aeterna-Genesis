...
latest strict burst: `dream-20260910-0812`
continuity digest: `cc5b2c50020f7e6f8849`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=1/5, control=11/16, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=33, pair-only=26, triads=23, split-asym=0.000162, no-split-asym=0.100195, energy-before-geometry=3. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-f023a067af` [strict/open-ended-followup] — X-f023a067af: status=REPEATED_SPECIFIC_CANDIDATE; same=27/53, nearby=?, control=0/53. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-deea6a8f25` [strict/open-ended-followup] — X-deea6a8f25: status=REPEATED_SPECIFIC_CANDIDATE; same=16/47, nearby=?, control=0/47. 回数だけでなく、何を変えると消えるかを追う。
