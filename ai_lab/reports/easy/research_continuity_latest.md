...
latest strict burst: `dream-20260822-0374`
continuity digest: `a558244a86af2e781cca`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=3/7, control=14/22, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=52, pair-only=43, triads=32, split-asym=0.08439, no-split-asym=0.297619, energy-before-geometry=1. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fc51cd23fe` [strict/open-ended-followup] — X-fc51cd23fe: status=REPEATED_SPECIFIC_CANDIDATE; same=0/6, nearby=?, control=0/6. 回数だけでなく、何を変えると消えるかを追う。
