...
latest strict burst: `dream-20260830-0545`
continuity digest: `c6bbbfb1dd493b7a4ba1`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=9/11, control=13/18, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=48, pair-only=40, triads=31, split-asym=0.376837, no-split-asym=0.138538, energy-before-geometry=2. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=12/30, nearby=?, control=0/30. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-f023a067af` [strict/open-ended-followup] — X-f023a067af: status=REPEATED_SPECIFIC_CANDIDATE; same=15/30, nearby=?, control=0/30. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-e64f84a189` [strict/open-ended-followup] — X-e64f84a189: status=REPEATED_SPECIFIC_CANDIDATE; same=10/11, nearby=?, control=0/11. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=71/88, nearby=?, control=0/88. 回数だけでなく、何を変えると消えるかを追う。
