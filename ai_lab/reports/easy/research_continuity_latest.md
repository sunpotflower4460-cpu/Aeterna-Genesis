...
latest strict burst: `dream-20260909-0789`
continuity digest: `62a44aad4b7ed25d6e62`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=5/6, control=14/16, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=34, pair-only=30, triads=23, split-asym=0.211451, no-split-asym=0.038062, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=23/51, nearby=?, control=0/51. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-e64f84a189` [strict/open-ended-followup] — X-e64f84a189: status=REPEATED_SPECIFIC_CANDIDATE; same=13/15, nearby=?, control=0/15. 回数だけでなく、何を変えると消えるかを追う。
