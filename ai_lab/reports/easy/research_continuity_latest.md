...
latest strict burst: `dream-20260904-0663`
continuity digest: `5872bb9ba344a198347f`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=9/12, control=13/17, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=56, pair-only=49, triads=32, split-asym=0.263494, no-split-asym=0.206977, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fc51cd23fe` [strict/open-ended-followup] — X-fc51cd23fe: status=REPEATED_SPECIFIC_CANDIDATE; same=2/16, nearby=?, control=0/16. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=18/42, nearby=?, control=0/42. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-f023a067af` [strict/open-ended-followup] — X-f023a067af: status=REPEATED_SPECIFIC_CANDIDATE; same=19/40, nearby=?, control=0/40. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-deea6a8f25` [strict/open-ended-followup] — X-deea6a8f25: status=REPEATED_SPECIFIC_CANDIDATE; same=10/34, nearby=?, control=0/34. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=83/102, nearby=?, control=0/102. 回数だけでなく、何を変えると消えるかを追う。
