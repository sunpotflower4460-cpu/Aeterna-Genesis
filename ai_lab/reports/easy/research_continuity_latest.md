...
latest strict burst: `dream-20260903-0646`
continuity digest: `82ad9388624765c79ab4`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=2/3, control=12/19, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=35, pair-only=29, triads=22, split-asym=0.085208, no-split-asym=0.098142, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fc51cd23fe` [strict/open-ended-followup] — X-fc51cd23fe: status=REPEATED_SPECIFIC_CANDIDATE; same=2/15, nearby=?, control=0/15. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=17/41, nearby=?, control=0/41. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-deea6a8f25` [strict/open-ended-followup] — X-deea6a8f25: status=REPEATED_SPECIFIC_CANDIDATE; same=10/31, nearby=?, control=0/31. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=82/100, nearby=?, control=0/100. 回数だけでなく、何を変えると消えるかを追う。
