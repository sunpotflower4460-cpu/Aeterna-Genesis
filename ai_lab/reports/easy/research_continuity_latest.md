...
latest strict burst: `dream-20260911-0849`
continuity digest: `e8138d7423ab015d5fc0`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=1/4, control=11/15, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=37, pair-only=30, triads=22, split-asym=0.046534, no-split-asym=0.257203, energy-before-geometry=1. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-ffb5d5b31a` [strict/open-ended-followup] — X-ffb5d5b31a: status=REPEATED_SPECIFIC_CANDIDATE; same=0/26, nearby=?, control=0/26. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=26/54, nearby=?, control=0/54. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-e64f84a189` [strict/open-ended-followup] — X-e64f84a189: status=REPEATED_SPECIFIC_CANDIDATE; same=17/19, nearby=?, control=0/19. 回数だけでなく、何を変えると消えるかを追う。
