...
latest strict burst: `dream-20260829-0530`
continuity digest: `273f1ee6530f4e32b1cc`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=2/4, control=9/16, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=28, pair-only=25, triads=21, split-asym=1e-06, no-split-asym=0.019872, energy-before-geometry=1. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=67/84, nearby=?, control=0/84. 回数だけでなく、何を変えると消えるかを追う。
