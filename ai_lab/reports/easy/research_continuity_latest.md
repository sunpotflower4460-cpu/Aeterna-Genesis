...
latest strict burst: `dream-20260921-1077`
continuity digest: `1d5310b2ddc00fd6f27c`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=4/8, control=12/13, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=31, pair-only=29, triads=21, split-asym=0.078509, no-split-asym=0.093118, energy-before-geometry=2. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-ffb5d5b31a` [strict/open-ended-followup] — X-ffb5d5b31a: status=REPEATED_SPECIFIC_CANDIDATE; same=1/30, nearby=?, control=0/30. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=38/75, nearby=?, control=0/75. 回数だけでなく、何を変えると消えるかを追う。
