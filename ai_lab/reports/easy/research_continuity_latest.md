...
latest strict burst: `dream-20260828-0515`
continuity digest: `95ead2ca90aa891f2fbf`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=2/4, control=12/16, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=28, pair-only=20, triads=23, split-asym=0.041703, no-split-asym=0.048035, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-deea6a8f25` [strict/open-ended-followup] — X-deea6a8f25: status=REPEATED_SPECIFIC_CANDIDATE; same=7/25, nearby=?, control=0/25. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=66/83, nearby=?, control=0/83. 回数だけでなく、何を変えると消えるかを追う。
