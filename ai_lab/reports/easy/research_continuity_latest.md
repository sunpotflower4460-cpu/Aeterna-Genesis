...
latest strict burst: `dream-20260907-0751`
continuity digest: `cccab17fcc028b12e4a7`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=1/2, control=10/17, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=35, pair-only=32, triads=20, split-asym=0.251976, no-split-asym=0.075458, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=22/49, nearby=?, control=0/49. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=87/107, nearby=?, control=0/107. 回数だけでなく、何を変えると消えるかを追う。
