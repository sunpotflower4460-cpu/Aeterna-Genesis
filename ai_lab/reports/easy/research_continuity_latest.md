...
latest strict burst: `dream-20260906-0713`
continuity digest: `1d430d70b530c20ccb1d`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=7/10, control=12/15, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=36, pair-only=29, triads=26, split-asym=0.308376, no-split-asym=0.296007, energy-before-geometry=2. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fc51cd23fe` [strict/open-ended-followup] — X-fc51cd23fe: status=REPEATED_SPECIFIC_CANDIDATE; same=2/18, nearby=?, control=0/18. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-deea6a8f25` [strict/open-ended-followup] — X-deea6a8f25: status=REPEATED_SPECIFIC_CANDIDATE; same=13/39, nearby=?, control=0/39. 回数だけでなく、何を変えると消えるかを追う。
