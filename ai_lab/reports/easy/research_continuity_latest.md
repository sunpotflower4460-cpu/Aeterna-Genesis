...
latest strict burst: `dream-20260920-1058`
continuity digest: `05ecbec605cbf56beaeb`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=3/5, control=19/29, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=51, pair-only=45, triads=35, split-asym=0.113545, no-split-asym=0.020525, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=35/72, nearby=?, control=0/72. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-f023a067af` [strict/open-ended-followup] — X-f023a067af: status=REPEATED_SPECIFIC_CANDIDATE; same=41/74, nearby=?, control=0/74. 回数だけでなく、何を変えると消えるかを追う。
