...
latest strict burst: `dream-20260822-0376`
continuity digest: `874d6326a1fb35345e37`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=4/4, control=7/11, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=29, pair-only=25, triads=15, split-asym=0.126353, no-split-asym=None, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=9/21, nearby=?, control=0/21. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-f023a067af` [strict/open-ended-followup] — X-f023a067af: status=REPEATED_SPECIFIC_CANDIDATE; same=9/21, nearby=?, control=0/21. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-deea6a8f25` [strict/open-ended-followup] — X-deea6a8f25: status=REPEATED_SPECIFIC_CANDIDATE; same=5/19, nearby=?, control=0/19. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=43/52, nearby=?, control=0/52. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-a428f23471` [strict/open-ended-followup] — X-a428f23471: status=REPEATED_SPECIFIC_CANDIDATE; same=2/6, nearby=?, control=0/6. 回数だけでなく、何を変えると消えるかを追う。
- `x-mechanism:X-dcc459d23a` [x-mechanism-exploratory] — 平均振幅で正規化したときXの特徴が消えるかを別seed・別介入で反証し、単純な増幅を未知構造と誤認していないか確認する。
