...
latest strict burst: `dream-20260826-0461`
continuity digest: `a8b1d91e2ded15ebae73`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=5/6, control=8/15, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=32, pair-only=26, triads=22, split-asym=0.080422, no-split-asym=0.058935, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-f023a067af` [strict/open-ended-followup] — X-f023a067af: status=REPEATED_SPECIFIC_CANDIDATE; same=11/24, nearby=?, control=0/24. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=56/70, nearby=?, control=0/70. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-d7d980689e` [strict/open-ended-followup] — X-d7d980689e: status=REPEATED_SPECIFIC_CANDIDATE; same=7/11, nearby=?, control=0/11. 回数だけでなく、何を変えると消えるかを追う。
