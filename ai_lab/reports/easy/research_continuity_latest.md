...
latest strict burst: `dream-20260826-0468`
continuity digest: `c9e2db10b9441fe3670b`

過去の重要点を次の研究へ渡す handoff です。元の証拠を置き換えません。

## Must carry forward

- `geometry:triangle-vs-control-separation` [strict-geometry] — 三角形を特別扱いせず対照と比較し続ける。 triangle=4/5, control=11/15, triangle_required=False.
- `energy:vertex-asymmetry-vs-geometry` [strict-local-energy] — 局所エネルギーは幾何で関係を選んだ後に測る。 pairs=31, pair-only=29, triads=20, split-asym=0.078744, no-split-asym=0.126763, energy-before-geometry=0. 因果・力・結合エネルギーとはまだ呼ばない。
...
- `x:X-ff29196d15` [strict/open-ended-followup] — X-ff29196d15: status=REPEATED_SPECIFIC_CANDIDATE; same=2/3, nearby=?, control=0/3. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-fc51cd23fe` [strict/open-ended-followup] — X-fc51cd23fe: status=REPEATED_SPECIFIC_CANDIDATE; same=2/11, nearby=?, control=0/11. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-fa3969eebd` [strict/open-ended-followup] — X-fa3969eebd: status=REPEATED_SPECIFIC_CANDIDATE; same=10/25, nearby=?, control=0/25. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-f023a067af` [strict/open-ended-followup] — X-f023a067af: status=REPEATED_SPECIFIC_CANDIDATE; same=12/25, nearby=?, control=0/25. 回数だけでなく、何を変えると消えるかを追う。
- `x:X-ee656d0a1c` [strict/open-ended-followup] — X-ee656d0a1c: status=REPEATED_SPECIFIC_CANDIDATE; same=2/3, nearby=?, control=0/3. 回数だけでなく、何を変えると消えるかを追う。
...
- `x:X-dcc459d23a` [strict/open-ended-followup] — X-dcc459d23a: status=REPEATED_SPECIFIC_CANDIDATE; same=58/72, nearby=?, control=0/72. 回数だけでなく、何を変えると消えるかを追う。
...
- `x-mechanism:X-b991d59a4d` [x-mechanism-exploratory] — drive-stronger のpaired効果を将来のholdout seedで再検証し、感度の向きと大きさが残るか壊しに行く。
