...
- **床（正直に）**：**L* 周りの有界 basin（大域的アトラクタではない）**——L* より遥かに小さく（未解像に）始めても、遥かに大きく始めても巻き戻る（L=40 で start_scale 3.2 は Q_H→0.39）。流れは**解像された L* 近傍**の始点を保つ。さらに**大 c4 は解像度ぎりぎり**：L*∝√c4 が格子カットオフに迫り、固定 L では c4 を無制限に上げられない（c4=40 は start 2.0 で Q_H≈1 だが 2.6 で≈0.66）。絶対 L* は解像度/κ 依存。robust（κ に~80×非依存）なのは「Q_H≈1 保持・有限 L*・L*(c4) 増加」。文献（Q_H=1 は Faddeev-Skyrme の安定最小化子）が裏づけ。「粒子」は analogy。
