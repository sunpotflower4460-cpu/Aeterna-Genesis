...
- **双方向（drag>0）**：背反応が流れを**自己制限**（U を 3.35 に抑え）粒子を保持（Q_H=0.97）＝創発的な
  負のフィードバック（ホメオスタシス）。一方向では出ない。
- **一方向（drag=0）**：背反応なし→同じ代謝が U を 9.56 まで過駆動→**粒子崩壊（Q_H=−0.21）**。
  **背反応が粒子を救う＝差が結合そのもの**（backreaction_saves_particle=True）。
- **駆動オフ（s=0）**：b→0.005→U→0→流れ停止→輸送停止（三体が一蓮托生。b の basal 分解で b→0 を担保）。
