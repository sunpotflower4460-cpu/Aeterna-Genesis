...
## 捕まえた罠（自分のバグ＝κ–解像度相互作用）
固定 κ の biharmonic フィルタ `1/(1+dt·κ·|k|⁴)` は、**高解像度で大 soliton を過減衰**する
（|k_max|=π/dx が細格子で増える）。素朴に L=56/κ=40 で回すと size が **sub-√c4**（R²≈0.57）に崩れた。
これは物理でなく**安定化子の過減衰**＝κ は dx⁴ でスケールすべき。L=44/κ=40 較正点で √c4 は保たれる
（CV=3.5%）。→ 「まずコードを疑う」を実践し、**安定化子の較正問題**を特定（H001 で κ 較正＋高解像度）。
