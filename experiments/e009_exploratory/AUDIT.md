...
| 項目 | tier | 測定 |
|---|---|---|
| クエンチ後、±渦は逆符号の方が近くにいる（対形成＝"分子") | **measured / frontier-observation** | 最近接 同符号 13.5 vs 逆符号 **8.6**（**最小像＝周期距離**、Codex P2）。置換検定 p<0.003＝引力で対を作る |

→ 「±渦が束縛した"分子/原子"を作るか?」に **yes（逆符号が有意に近い、符号入替え帰無に対し p<0.003）**。束縛状態の証明ではなくスナップショット統計（床）。距離は**周期境界＝最小像**で測定（境界越えの隣接を遠いと誤らない）。
