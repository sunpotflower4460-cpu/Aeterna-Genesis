...
- **相図は臨界駆動曲線 s_c(dA)**：bracket された行（下に死サンプルがある行）(dA,s_c)=[(0.25,0.05),(0.35,0.1)]＝
  strictly rises。dA=0.15 は最小駆動でも生（閾値が走査域外）＝報告のみ、gate に使わない＝
...
- **駆動で持続・切ると溶ける（死）**、**漏れ↑→生存に必要な駆動↑**（leak 0→0.4→0.8 で min s 0.1→0.2→0.2、生存判定=bounded_single_vesicle）
