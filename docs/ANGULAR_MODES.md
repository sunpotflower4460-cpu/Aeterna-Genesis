...
### M2 → 3D：測定器を次元非依存化（球面調和 ℓ ＋ 3 並進 Goldstone）

`coupled_spectrum` を**次元非依存**にした（2D の挙動は不変・M2-A〜D 再現）。3D では：
- **並進 Goldstone が 3 本**（`Tx, Ty, Tz` = 全場の ∂x/∂y/∂z・計量正規化）。
- **角モードは球面調和の次数 ℓ** で分類（`ℓ=0` 呼吸／`ℓ=1` 並進=drift／`ℓ=2` 分裂=split）。球面殻で
  サンプルし実 SH（ℓ=0,1,2）へ射影。→ **drift-before-split の判定（非 Goldstone ℓ=1 が ℓ=2 より先）が
  そのまま 3D で使える**（`classify_drift_before_split` は m でも ℓ でも動く）。
- 検証：構成場で ℓ=0/1/2 を厳密分類・3D で Goldstone 3 本・3D コード経路が eigs まで通る（`tests/test_coupled_spectrum_3d.py`）。
- **honest floor**：これは**測定器の 3D 化**であって、**3D の drift 測定そのものではない**。実測には
  「3D で安定 compact 単一個体に落ち着く白」が要る（M2 の scissors・M2-E-3D＝次段）。
