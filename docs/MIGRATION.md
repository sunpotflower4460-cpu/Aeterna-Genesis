...
- **次元（正直に）**：これは**検証済み 2D** Room。3D 昇格は別段階の監査（DIMENSION_POLICY）＝2D 成功を自動外挿しない。3D primitive-variable ソルバは 3/2 ゼロ詰め de-aliasing を入れるまで非線形項が飽和しない（2/3 truncation だけでは格子スケール aliasing が蓄積）ため frontier。剛体壁・高 Ra 時間依存対流も別 Room。
