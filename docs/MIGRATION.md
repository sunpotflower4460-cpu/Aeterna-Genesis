...
## Observatory UI 刷新 — Phase 0：場記録パイプライン（別PR・物理不変）

現状の Room はスカラー要約と最終場 checksum のみ保存し、**実測の場・時系列を持たない**。UI で「t=0 から育つ現象を再生／観測レンズ切替」を**偽の粒子でなく実測で**行うため、記録パイプラインを先に追加した（`genesis/recording/recorder.py`）。

- **やること**：各正式 Room の seed-0 について、シミュレーションが**すでに計算している場**を ~12 時刻でスナップショットし、表示用に間引き（2D→48²／3D→20³・`scipy.ndimage.zoom`）＋ uint8 量子化（[vmin,vmax] 保存）して `runs/seed-0000/field.json` に、レンズ→実測物理量の対応（単位・変換・クリッピング・honesty）を `render-manifest.yaml` に保存。巨大データは **catalog.json に入れず**、`catalog → room manifest → field.json` の参照構造にする（catalog は 22KB のまま）。
- **実測レンズのみ**：G001=phase(arg ψ)/density(|ψ|²)、G002=temperature(θ)/velocity(|u|)/vorticity(∇×u)、G003=composition(φ)/boundary(|∇φ|)/velocity(|u|)。存在しない量は出さない。
- **物理不変（検証済み）**：記録は場を**読むだけ**。recorder あり/なしで run の checksum が**ビット一致**、3 Room を再ビルドしても既存 checksum は**全て不変**（`changes_physics:false`, `decorative_particles:false` を schema と test で強制）。
- **配線**：`schemas/field-index.schema.json` + `render-manifest.schema.json`（CI-B で検証）、`build_catalog.py` に参照追加、`tests/test_recording.py`（roundtrip・honesty・各 Room の場が decode 可能）、CI で「参照であって埋め込みでない・honesty」を検査。

次は Phase 1（React+TS+Vite+R3F の Visual Prototype）。

## Observatory UI 刷新 — Phase 1：Visual Prototype（React + Vite + R3F・物理不変）

指示書の刷新方針（3D 現象が主役・段階的開示・表示と始原条件の分離・偽装禁止・catalog 駆動）を、
承認済みの設計案（Artifact）に沿って実装。**物理・solver・監査・正式 Room 結果には一切触れない。**

- **技術**：React + TypeScript + Vite + React-Three-Fiber。依存はローカルにバンドル（CDN 不使用）、
  `base:'./'` で自己完結の静的ビルド（オフライン成立）。旧単一 HTML は `app/legacy.html` に保存。
- **catalog 駆動**：`tools/collect_app_data.py` が `catalog.json` ＋ 各 Room の `field.json`（Phase 0 の記録）
  ＋ `render-manifest.json` を `app/public/data/` へ集約。UI はこれを読むだけ（ハードコードなし）。巨大データは
  catalog に入れない参照構造を維持。
- **本物の実測場を再生**：`field.json`（uint8 量子化 base64）を復元し、2D は面・3D は体積の点群として、
  観測レンズ（G001 phase/density・G002 temperature/velocity/vorticity・G003 composition/boundary/velocity）に
  対応する色で表示。偽の粒子・見栄えのための流れは描かない。実測 RB の対流プルーム・TDGL の位相場が再生される。
- **表示 ↔ 始原条件の分離**：View（閾値/透明度/発光）は即時・物理不変。Genesis は「保留中」に貯め、
  「新しい Room として t=0 実行」だけ（正式 Room の途中状態を誘導しない。Live Runner は Phase 3）。
- **Physics Integrity 3 段階**：やさしい概要 → 確認項目 → 研究者向け数値（監査は削除せず・常時全面に出さない）。
- **画面**：Universe Lobby（カード・バッジ）／Room Workspace（ビューポート・レンズ・再生/時間移動・HUD・
  下部コントロール・Branch Room）／Inspector（開いても 3D を見続けられる）／モバイル対応。

検証：オフライン `npm run build` 成功、`tsc --noEmit` クリーン、ヘッドレス Chromium で Lobby・Room（2D 面/3D 体積）・
Inspector が JS エラーなく描画（実測場が再生されることをスクリーンショットで確認）。次は Phase 2/3/4。

## Observatory UI 刷新 — Phase 2：Recorded Data Integration（物理不変）

Phase 1 の記録データ再生を作り込み。**物理・監査・正式 Room 結果には触れない。**

- **Web Worker で場処理をオフスレッド化**（`app/src/lib/decode.worker.ts`）：`field.json` 取得と
  base64→Float32 復元を UI スレッドから分離。base URL は絶対化し worker の fetch がページ基準で解決。
  worker 非対応環境はメインスレッド復元へフォールバック。
- **滑らかな再生**：再生ヘッドを R3F レンダーループ（`useFrame`）で進め、記録フレーム間を線形補間（cyclic な
  位相は継ぎ目回避のため最近傍）。React 再レンダーなしで毎フレーム更新。スクラブ/リセット/速度と同期。
- **LOD・品質・fallback・アクセシビリティ**：View タブの「表示品質 (LOD)」＝3D 点群の間引きと DPR を制御
  （低性能端末向け・物理解像度と無関係）。WebGL2 非対応は fallback 表示。`prefers-reduced-motion` で自動回転停止。
- **Parallel / Compare View（§10）**：2 Room を**同じ時刻・同じ観測レンズ**で左右に並置し共有クロックで同期再生。
  差分（到達 Level・次元・genesis model・バッジ）を表示。Lobby から起動。
...
## 改善キャンペーン — Phase A1：Evidence Contract v2 前登録（docs-only・物理/数値/CI 不変）

外部監査（指示書・基準コミット `8d8dedc`）が挙げた「metadata の畳み込み」——`official` が **"検証済み"** と
**"3D 昇格完了"** を混同、`physics_status.conservation` に **厳密保存/収支則/Lyapunov/独立推定一致/数値残差**の
5 種が混在、`reached_level` と能力ベクトルの二重の真実——を、**実装前に語彙で解く**前登録。

**追加した（docs のみ）**：
- `docs/EVIDENCE_CONTRACT_V2.md`：二軸（**証拠検証軸** unvalidated/validated_2d/validated_local_3d/
  validated_global_3d ＋ **昇格軸** exploration/candidate/promotion_ready/official/rejected_in_3d）と
  **official の機械検証可能な不変条件**（official ⇒ dimension3 ∧ full_3d passed ∧ convergence ∧ reproducibility ∧
  balance_or_invariant ∧ integrity_audit）。Level は能力ベクトルの保守的要約（`localization`→pattern/
  interface/compact_individual、`motion`→material_flow/circulation/pattern_translation/self_propulsion）。
  保存を 5 種へ区別。claim tier 改定方針（G001=lower-bound proxy／G002・G003=validated 2D／agent_reaction／
  Model H budget）。semantic lint 仕様（入力・7 ルール・負 fixture・warning→error）。閾値前登録の様式。
  **G001/G002/G003/agent_reaction 移行表**。
- `docs/EVIDENCE_CONTRACT_V2_AUDIT.md`：独立監査（docs-only の確認・v2 の限界・次の停止条件）。
- 本 `MIGRATION.md` に本節を追記。

**変更しなかった（不変条件）**：`schemas/`・`rooms/`（room.yaml・emergence.json・checksum・field.json・
run manifest）・`genesis/`（物理コード・`measures.py`＝no_touch）・`experiments/*/experiment.yaml`・`app/`・
`.github/workflows/ci.yml`・`tools/`。**数値・checksum・CI に差分なし**（docs-only）。

**帰結（A2 で実装）**：`room-g002-a`/`room-g003-a` は official-3D の不変条件を満たさないため、**「失敗」でなく
`validated_2d`＋`candidate`** へ言い換える（数値・folder・checksum は不変）。`room-g001-a` は full-3D なので
`balance_or_invariant` を **Lyapunov 単調性**で正しく名付け直して official 維持可否を機械検証する。

**まだやらない**：semantic validator の機械化・metadata の実書き換え・role 監査 41 件化・provenance schema・
App CI は **Phase A2**（レビュー後）。閾値の感度分析数値は A2 / Phase F。**A1 は前登録のみで停止する。**
