...
- **第三 → 粒子**：四次 Skyrme 項が Q_H=1 ホップ粒子を有限 L* で安定化。**完全PDE 自己安定化**（半陰的、エネルギー単調、c4>0 が Q_H≈1 保持、e012 Stage3）。**Q_H=2 も同じ流れで保持**（|Q_H|≈2、e016、observed）。**~~size=k√c4~~ は第8監査で撤回**（start∝√c4 埋込・流れ未収束＝target_encoded／decoupling で √c4 消滅。真の start 非依存 L* は frontier/H001）。
