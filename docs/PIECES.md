...
- **H2 法則クラスで Level を登る（深い Level は別の法則が要る）** `measured` — 「同じスコアを上げる」のでなく
  **法則クラスを変える**と 0 から届く Level が変わる（`ai_lab/lab.py --mode lawscan`、`genesis/models/`＋
  ...
  - **swift_hohenberg（双安定パターン形成・SH35）**：対称バンプ＋ノイズ → **持続・自己修復する局在個体**。
    地図の欠けていた段 **L4（持続する個体性）** を measured で満たす（内外差 amax~1.4・凍結 attractor・上半分を
    消しても**再成長**＝自己修復・N=64/96/128 同一＝サイズ非依存）。**自己修復は"置けない"**（L2 凍結欠陥と個体の
    決め手）。**能力ごとの正直**（GPT②）：自己修復・持続・サイズ非依存は emerged だが**局在は seeded**（bump を置く）
    ＝「0 から個体が創発」でなく「置いた局在 seed が自己修復 attractor に育った」（`GENESIS_PROVENANCE.md`）。
    ただし**変分**なので個体は**動かない**（重心ドリフト 0）＝**L4-static**。**自走する個体（L4∧運動）は
    frontier**。**個体性(L4)と自発運動(L3)は独立な二軸**（`WHITE_CEILINGS.md` 二軸の再枠づけ）。同じ数学 ≠ 同じもの
    （局在 attractor であって細胞・生命ではない）。
