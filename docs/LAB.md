...
- つまみ（と seed）の組み合わせを、1 回に最大 24 通り、t=0 から回す。
  - 1 コマごとに測る。測り方も判定も水槽と同じ。
  - 別々のプロセスで並行に回す。
- 結果は、ゴールの条件に近い順に並ぶ。ゴールの「まとめて試した結果」で見られる。
- 気になった組み合わせは「水槽に出す」で、t=0 から同じように動く宇宙として並べられる。
  - どの組み合わせもレシピ（白＋seed＋つまみ）と sha256 が残り、あとで replay できる。
- 計算量はゴールのステップ予算から引く。足りないときは、何コマなら入るかを返して断る。

### やさしい説明

ゴールのいちばん上に、はじめての人向けの説明を出す（`tools/lab/plain.py`）。

- 何をさがしているか、見つかったと言える目安、いくつ試したか、いちばん近いもの、いま誰が何をしているか、予算をどれだけ使ったか。
- **ラボが持っている数字だけから、機械的に作る**。AI もキーも要らず、推測は入らない。
- 「AI にもっとやさしく説明してもらう」を押すと、中心の AI がこの説明とマップを言いかえる。
  - 送るのは、ゴールの文と数字だけ。
  - 「生きている」「脳ができた」などと言いかえないように指示している。
- 研究員は、マップに書くときと終えるときに、専門用語を使わない言いかえ（`plain`）も書く。マップはそれを先に見せ、くわしい文は押すと出る。
