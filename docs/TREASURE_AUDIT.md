...
- 見出しだった「名無し変化 X-…」は、t=0 から回し直した 567 回の出現のうち **565 回（99.6%）が通常のクエンチ物理**だった（振幅の立ち上がり・渦数の変化・緩和・対消滅）。説明できなかったのは 2 回で、宝候補として残す（[`X_PATTERN_AUDIT.md`](X_PATTERN_AUDIT.md)）。

**正直なまとめ**：2 か月・約 4,000 コミットの自動研究は、天井がわかっている白 1 つの、ごく狭い条件空間を繰り返し回していた。
新しい Level は出ていない。説明のつかない現象は、再実行で出た 567 回のうち 2 回だけだった。これは「失敗」ではなく**負の結果として有用**である。
...
4. **負の結果**：TDGL は約 7,000 run でも L2 を越えない。X パターンの再実行出現の 99.6% は、既知の物理で説明できる。
5. **X の宝候補 2 件**（X-0a3002e1aa、X-29d79aabf4）：平坦値の近くで、渦数を変えずに振幅のばらつきが揃う出来事。緩和かどうかを、位置の追跡と自由エネルギーの時系列で測る価値がある。
