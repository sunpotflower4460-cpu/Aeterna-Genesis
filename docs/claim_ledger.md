...
| # | 主張 | tier | 裏づけ |
|---|---|---|---|
| 1 | 同符号の二渦は GPE だけから互いの周りを回り合う（共回転、間隔ほぼ一定） | **measured** | `result.json`：累積回転 282°（単調増加）、間隔 mean 14.70（13.77–16.13）、中心移動 0.00 |
...
| 4 | エネルギー・ノルムが保存（一様背景・周期境界の保存系） | **measured** | 同符号 E/N drift 1.7e-5/1.4e-12、逆符号 2.4e-5/3.4e-13 |
| 5 | 挙動の質（同符号=共回転／逆符号=並進）は間隔 d を変えても保たれる | **measured** | `robustness.json`：6/6 ケース PASS（同符号 d∈{6,7,9}・逆符号 d∈{12,16,20}） |
...
| 8 | 同符号対は静止解でないため late-time に音波が蓄積（クリーン窓で測定） | **interpretive** | AUDIT.md §5（クリーン窓と劣化を隠さない） |
