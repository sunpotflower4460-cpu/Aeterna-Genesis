...
951回、64個の独立seed、64条件で観測。ただし広く出すぎるため、今後は回数を増やすより『何を変えると消えるか』を優先します。
...
持続2渦 35件（pair-only 27件）、3渦の局所エネルギー 23件。形だけで関係を選んだ後にエネルギーを測っています。
...
- F-pathだけを追わず、132種類規模の名無し反復から条件特異的なものを優先して壊す。
