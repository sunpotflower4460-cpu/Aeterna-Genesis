...
7297回、64個の独立seed、64条件で観測。ただし広く出すぎるため、今後は回数を増やすより『何を変えると消えるか』を優先します。
...
持続2渦 36件（pair-only 30件）、3渦の局所エネルギー 23件。形だけで関係を選んだ後にエネルギーを測っています。
...
再現が弱かったX候補 106件、Deep-Time隔離メモ 43件などを機械向けに保持。人向け画面では失敗詳細を広げませんが、証拠は削除しません。
